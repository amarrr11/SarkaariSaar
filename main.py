...
if process_btn and url_input:
    ...
    if len(docs) == 0:
        ...
        vectorstore = process_docs(docs)
        st.success("✅ Content processed and saved!")
...
faiss_store_exists = os.path.exists("faiss_store/index.faiss")

if user_question:
    if not faiss_store_exists:
        st.error("❌ Please process some URLs first to enable Q&A.")
    else:
        try:
